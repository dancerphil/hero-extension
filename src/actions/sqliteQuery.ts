...
const sqliteQuery = async () => {
    ...
    const embeddingResult = embeddings.search.all(new Float32Array(embedding));
    const ids = embeddingResult.map(item => item.cci_id);
    const cciResult = codeContextItems.select.all(...ids);
